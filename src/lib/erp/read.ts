...
export async function snapshot(userId:string){
 ...
 const [skus,warehouses,customers,suppliers,balances,orders,receipts,events,employees,batches]=await Promise.all([
  tx.inventorySku.findMany({orderBy:{code:'asc'}}),tx.warehouse.findMany({orderBy:{code:'asc'}}),
  ...
  financial?tx.inventoryBatch.findMany({relationLoadStrategy:'join',include:{sku:true,receiptLine:{include:{receipt:{include:{warehouse:true}},purchaseLine:{include:{purchase:{include:{supplier:true}}}}}},balances:{include:{warehouse:true}}},orderBy:[{receivedAt:'desc'},{id:'asc'}]}):[],
 ]);
 ...
 const batchReports=batches.filter(b=>b.receiptLine?.receipt.state==='POSTED').map(b=>{
  ...
  const rows=orders.flatMap(o=>o.shipments.filter(s=>s.state==='POSTED').flatMap(s=>s.allocations.filter(a=>a.batchId===b.id).map(a=>{
   ...
   return {id:a.id,order:o.number,customer:o.customer.name,date:s.postedAt,actor:s.actor.displayName||s.actor.email,warehouse:s.warehouse.name,quantity:a.quantity,price:a.unitPrice.toString(),currency:o.currency,fx:o.fxToAud,receivable:due,paid,due:due.sub(paid)};
  })));
  const known=!!b.unitCostAud&&rows.every(r=>r.fx!==null);const shipped=rows.reduce((s,r)=>s+r.quantity,0);
  const revenue=sum(rows.map(r=>r.receivable.mul(r.fx||0)));const paid=sum(rows.map(r=>r.paid.mul(r.fx||0)));const cost=(b.unitCostAud||zero()).mul(shipped);
  return {id:b.id,batch:b.batchNo,label:b.label,sku:b.sku.code,name:b.sku.name,unit:b.sku.unit,date:b.receivedAt,supplier:b.receiptLine!.purchaseLine.purchase.supplier.name,warehouse:b.receiptLine!.receipt.warehouse.name,note:b.receiptLine!.receipt.note,currency:b.currency,fxToAud:b.fxToAud?.toString()||null,unitCost:b.unitCost.toString(),totalCost:b.unitCost.mul(received).toFixed(2),received,shipped,remaining:b.balances.reduce((s,v)=>s+v.quantity,0),warehouses:b.balances.map(v=>({name:v.warehouse.name,quantity:v.quantity})),revenue:known?revenue.toFixed(2):null,paid:known?paid.toFixed(2):null,due:known?revenue.sub(paid).toFixed(2):null,cost:known?cost.toFixed(2):null,profit:known?revenue.sub(cost).toFixed(2):null,rows:rows.map(r=>({...r,fx:undefined,receivable:r.receivable.toFixed(2),paid:r.paid.toFixed(2),due:r.due.toFixed(2)}))};
 });
 const inventory=skus.map(s=>({id:s.id,code:s.code,name:s.name,group:s.groupName,unit:s.unit,warehouses:warehouses.map(w=>({id:w.id,name:w.name,quantity:balances.filter(b=>b.batch.skuId===s.id&&b.warehouseId===w.id).reduce((v,b)=>v+b.quantity,0)})),total:balances.filter(b=>b.batch.skuId===s.id).reduce((v,b)=>v+b.quantity,0)}));
 ...
 return JSON.parse(JSON.stringify({inventory,groups,batches:batchReports,customerSummaries:financial?summaries:[],totals:{due:financial&&!hasUnknown?allDue.toFixed(2):null,profit:financial&&!hasUnknown?allProfit.toFixed(2):null},user:{name:user.displayName||user.email,manager,financial,permissions:['sku','warehouse','customer','supplier','receive','sale','ship','payment','reverse','employee'].filter(a=>can(user,a))},skus:skus.map(s=>({id:s.id,code:s.code,name:s.name,group:s.groupName,unit:s.unit,price:s.salePrice.toString()})),warehouses,customers,suppliers,balances:balances.map(b=>({sku:b.batch.sku.code,name:b.batch.sku.name,warehouse:b.warehouse.name,batch:b.batch.batchNo,quantity:b.quantity,currency:b.batch.currency,cost:financial?b.batch.unitCost.toString():null,receivedAt:b.batch.receivedAt})),orders:sales,receipts:receipts.map(r=>({id:r.id,number:r.number,state:r.state,note:r.note,time:r.postedAt,warehouse:r.warehouse.name,actor:r.actor.displayName||r.actor.email,lines:r.lines.map(l=>({sku:l.batch.sku.code,batch:l.batch.batchNo,label:l.batch.label,date:l.batch.receivedAt,currency:l.batch.currency,quantity:l.quantity,cost:l.batch.unitCost.toString(),supplier:l.purchaseLine.purchase.supplier.name}))})),events:events.map(e=>({id:e.id,action:e.action,entityId:e.entityId,reason:e.reason,time:e.createdAt,actor:e.actor.displayName||e.actor.email})),employees}));
