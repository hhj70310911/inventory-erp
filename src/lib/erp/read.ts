...
export async function snapshot(userId:string){
 ...
 const [skus,warehouses,customers,suppliers,balances,orders,receipts,events,employees,batches]=await Promise.all([
  ...
  manager?tx.user.findMany({where:{OR:[{role:'ADMIN'},{employeeRole:{not:null}}]},select:{id:true,email:true,displayName:true,employeeRole:true,role:true,active:true,updatedAt:true}}):[],
