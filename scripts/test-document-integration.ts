...
async function main(){
 ...
 const {snapshot}=await import('../src/lib/erp/read');const stats=await snapshot(admin.id);assert.equal(stats.dailyProfit.find((d:{shipments:number})=>d.shipments>0).profit,'139.99');await db.user.update({where:{id:worker.id},data:{active:true}});assert.equal((await snapshot(worker.id)).dailyProfit,null);await db.user.update({where:{id:worker.id},data:{active:false}});
